  ...
  constructor(props) {
    ...
    for (var i=0; i < 100; i++) {
        ...
        if(params.hasOwnProperty("courses[" + i + "]['saoid']")){
          ...
          let link = "https://register2.artofliving.org/us/civicrm/event/register?id=" + params["courses[" + i + "]['start']"] + "&lcMessages=en_GB&reset=1&discount=" + params["code"];
    ...
    tick() {
        //if (typeof this.props.endDate  === typeof new Date()) {
            this.afterEachSecond(moment(this.state.expire, 'MM/DD/YYYY'));
        //} else {
        //}
    }
              ...
              <div className="landing_find_course_slider">
                
                {
                  this.state.cources.map((object, index) => {
                      return (
                          <div className="find_course_slide">
